...
install:
	mkdir -p "$(SKILL_DEST_AGENTS)" "$(SKILL_DEST_CLAUDE)"
	rsync $(RSYNC_FLAGS) ./ "$(SKILL_DEST_AGENTS)/"
	rsync $(RSYNC_FLAGS) ./ "$(SKILL_DEST_CLAUDE)/"
	...
	ln -s "$(SKILL_DEST_CLAUDE)/$(SANDCASTLE_SKILL_NAME)" "$(SANDCASTLE_SKILL_DEST_CLAUDE)"
